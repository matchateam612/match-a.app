...
import { createVoiceAssistantTranscript } from "@/app/onboarding/4-agent/_lib/agent-server";
...
export async function POST(request: Request) {
  try {
    ...
    if (!isValidRequestBody(body)) {
      return jsonError("Invalid voice transcript payload.");
    }
    ...
    return NextResponse.json(await createVoiceAssistantTranscript(body), { status: 200 });
  } catch (error) {
    return jsonError(
      error instanceof Error ? error.message : "The voice transcript could not be created.",
      500,
    );
  }
}
